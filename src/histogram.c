...
static void column_mean(void);
static void compute_correl(void);
static void compute_fourier(void);
static void compute_hist(void);
static void compute_power(void);
static void compute_sd(void);
static void compute_stacor(void);
...
static int two_d_hist(int col1,int col2,int ndat,int n1,int n2,double xlo,double xhi,double ylo,double yhi) {
	...
	norm=1./(double)ndat;
	/* now fill the data with the bin values - take the midpoints of each bin */
	for(i=0;i<n1;i++) {
		for(j=0;j<n2;j++) {
