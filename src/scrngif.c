...
static unsigned char *AddCodeToBuffer(int code, short n, unsigned char *buf);
static void ClearTree(int cc, GifTree *root);
static int gif_encode(FILE *fout, unsigned char *pixels, int depth, int siz);
static void gif_loop(FILE *fout, unsigned int repeats);
...
static int gif_encode(FILE *fout, unsigned char *pixels, int depth, int siz) {
	...
	buffer = (unsigned char *)malloc((BUFLEN+1)*sizeof(unsigned char));
	if(buffer == NULL ) {
		return 0;
	}
	buffer++;
	...
	topNode = baseNode = (GifTree *)malloc(sizeof(GifTree)*4094);
	if(topNode == NULL ) {
		return 0;
	}
	nodeArray = first->node = (GifTree **)malloc(256*sizeof(GifTree *)*noOfArrays);
	if(nodeArray == NULL ) {
		return 0;
	}
...
static void make_gif(unsigned char *pixels,int cols,int rows,FILE *dst) {
	...
	*pos++ = 0x7&(depth-1);
	...
	/* header info done */
	gif_encode(dst,pixels,depth,rows*cols);
	fputc(';',dst);
	free(buffer-1);
}
