...
void renamef(char *old, char *new_file) {
	rename(old,new_file);
}
...
void copyf(char *old, char *new_file) {
	FILE *fo,*fn;
	int c;
	fo=fopen(old,"r");
	fn=fopen(new_file,"w");
...
void appendf(char *old_file, char *new_file) {
	FILE *fo,*fn;
	FILE *ft;
	int c;
...
void create_auto_file_name(void) {
...
void three_parameter_homoclinic(void) {
	Auto.irs=grabpt.lab;
	Auto.itp=grabpt.itp;
